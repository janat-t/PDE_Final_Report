...
for i in tqdm(np.arange(0.0,end_time+dt, dt)):
	...
	n+=1
	signu = np.int(np.sign(u))
	xiiup = -signu*dx
	E = -u*dt
	a = -2*(np.roll(f,signu,axis=0)-f)/xiiup**3+(g+np.roll(g,signu,axis=0))/xiiup**2
	b = -3*(f-np.roll(f,signu,0))/xiiup**2-(2*g+np.roll(g,signu,0))/xiiup
	f = a*E**3+b*E**2+g*E+f
	g = 3*a*E**2+2*b*E+g
