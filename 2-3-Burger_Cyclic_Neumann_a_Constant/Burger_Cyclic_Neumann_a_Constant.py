...
ax.plot_surface(X[1:-2,:], Y[1:-2,:], U[1:-2,:], rstride=5, cstride=5, cmap='viridis', edgecolor='none')
ax.set_xlabel('x')
ax.set_ylabel('y')
ax.set_zlabel('U')
ax.set_zlim([-0.1,1.1])
...
for it in tqdm(np.arange(0.0,end_time + dt, dt)):
    # Neumann Boundary Condition for x-axis Boundaries
    U[0,:] = U[2,:]
    U[-1,:] = U[-3,:]
    ...
    if n%100==0:
        ...
        plt.cla()
    n += 1
    U = U + d2*(np.roll(U,-1,axis=1)+np.roll(U,1,axis=1)+np.roll(U,-1,axis=0)+np.roll(U,1,axis=0)-4.0*U) - d1*(2.0*U-np.roll(U,1,axis=0)-np.roll(U,1,axis=1))
